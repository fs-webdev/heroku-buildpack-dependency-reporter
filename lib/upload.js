...
try {
  ...
  if (!fs.existsSync("package-lock.json")) {
    console.log(
      `Attempting to generate package-lock.json file for ${APP_NAME} in ${TARGET_ENV}...`
    );
  ...
  // Upload to S3 depending on build env -- if TARGET_ENV is beta, upload to dev bucket instead
  console.log(
    `Uploading package-lock.json file to S3 for ${APP_NAME} in ${TARGET_ENV}...`
  );
  const params = {
    Bucket: `${AWS_BUCKET_NAME}-${TARGET_ENV === "beta" ? "dev" : TARGET_ENV}`,
  ...
  const result = s3.upload(params, (err, data) => {
    if (err) {
      console.log("Error uploading package-lock.json file to S3", err);
    } else {
      console.log("Successfully uploaded package-lock.json file to S3", data);
    }
  });
} catch (error) {
